...
export default function Home({ setPage }: HomeProps) {
  ...
  return (
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#ffed99' }}>

      {/* ── Hero ── */}
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">

        {/* Radial hero glow (commented out as it might not be needed with a light background) */}
        {/*
        ...
        */}
        ...
        {/* Concentric rings (optional, you can keep or remove them) */}
        ...
        <div className="relative z-10 text-center px-4 max-w-3xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
            ...
            style={{ background: 'rgba(245,204,0,0.1)', border: '1px solid rgba(245,204,0,0.25)', color: '#D4A800' }}>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div className="relative flex items-center justify-center">
              ...
              {/* Logo element without the background circle */}
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <span className="text-lg font-medium" style={{ color: 'rgba(139,94,0,0.6)', fontFamily: "'Playfair Display',serif" }}>Welcome to</span>
          </motion.div>
          ...
          <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.14 }}
            ...
            <span style={{ color: '#8B5E00' }}>Vikram</span>
          ...
          <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            ...
            style={{ color: 'rgba(139,94,0,0.65)' }}>
      ...
      {/* ── Divider ── */}
      <div className="h-px mx-10" style={{ background: 'linear-gradient(90deg,transparent,rgba(139,94,0,0.2),transparent)' }} />
      ...
      <section className="py-20 px-4 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10">
          <h2 className="text-4xl font-black mb-2"
            style={{ fontFamily: "'Playfair Display',serif", color: '#8B5E00' }}>Shop by Category</h2>
          <p className="text-sm" style={{ color: 'rgba(139,94,0,0.5)' }}>100+ varieties across all categories</p>
        </motion.div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {categories.map((cat, i) => (
            <motion.button key={cat.label}
              ...
              style={{ height: 200, boxShadow: '0 4px 24px rgba(0,0,0,0.1)', border: '1px solid rgba(139,94,0,0.1)' }}>
              <img src={cat.img} alt={cat.label}
                className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
              <div className="absolute inset-0" style={{ background: 'linear-gradient(to top,rgba(255,237,153,0.9) 0%,rgba(255,237,153,0.2) 60%,transparent 100%)' }} />
              {/* Gold top line */}
              <div className="absolute top-0 left-0 right-0 h-px" style={{ background: 'linear-gradient(90deg,transparent,rgba(139,94,0,0.5),transparent)' }} />
              <div className="absolute bottom-0 left-0 right-0 p-4">
                <div className="text-xl mb-1">{cat.emoji}</div>
                <h3 className="font-black text-white text-base leading-tight">{cat.label}</h3>
                <p className="text-xs mt-0.5" style={{ color: 'rgba(139,94,0,0.65)' }}>{cat.desc}</p>
              </div>
            </motion.button>
          ))}
        </div>
      ...
      {/* ── Features ── */}
      <section className="py-14 px-4" style={{ background: 'rgba(0,0,0,0.02)' }}>
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-black text-center mb-10"
            style={{ fontFamily: "'Playfair Display',serif", color: '#8B5E00' }}>Why Vikram Crackers?</h2>
            ...
            {features.map((f, i) => (
              <motion.div key={f.title}
                ...
                <div className="w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3"
                  style={{ background: 'rgba(139,94,0,0.05)', border: '1px solid rgba(139,94,0,0.1)' }}>
                  <f.icon size={22} style={{ color: '#D4A800' }} />
                </div>
                <h3 className="font-bold text-sm mb-1.5" style={{ color: '#8B5E00' }}>{f.title}</h3>
                <p className="text-xs" style={{ color: 'rgba(139,94,0,0.55)' }}>{f.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-4">
        <motion.div initial={{ opacity: 0, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
          ...
          style={{
            background: 'linear-gradient(135deg,rgba(139,94,0,0.9),rgba(184,130,0,0.85))',
            border: '1px solid rgba(139,94,0,0.2)',
            boxShadow: '0 12px 50px rgba(0,0,0,0.1)',
            backdropFilter: 'blur(12px)',
          }}>
          <div className="absolute inset-0 pointer-events-none" style={{
            background: 'radial-gradient(ellipse 60% 50% at 50% 30%, rgba(255,237,153,0.08) 0%, transparent 70%)',
          }} />
          <div className="absolute top-0 left-0 right-0 h-px" style={{ background: 'linear-gradient(90deg,transparent,rgba(255,237,153,0.5),transparent)' }} />
          <h2 className="relative text-3xl font-black mb-3"
            style={{ fontFamily: "'Playfair Display',serif", color: '#ffed99' }}>Ready to Celebrate? 🎉</h2>
          <p className="relative text-base mb-7" style={{ color: 'rgba(255,237,153,0.65)' }}>Browse our full catalog and place your order via WhatsApp in seconds.</p>
